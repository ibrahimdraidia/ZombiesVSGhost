...
/* Variable globale dans laquelle on memorise les parametres internes */
Ez_X ezx;

/* Permet de savoir si ez_init et ez_main_loop ont ete appele'es */
...
int ez_init ()
{
    ...
    /* Cree un xid pour memoriser les infos dans la fenetre */
    ...
    /* Propriete pour memoriser les infos dans la fenetre */
    ...
    ezx.mv_win = None;    /* pour filtrer mouvements souris */
...
Window ez_window_create (int w, int h, const char *name, Ez_func func)
{
   ...
   /* Place un masque d'evenements */
    ...
    /* Pour memoriser les infos de la fenetre */
    ...
    /* Associe les infos a la fenetre avec un xid ou une propriete */
...
void ez_window_show (Window win, int val)
{
    ...
    /* Si on n'est pas dans la main_loop, on differe l'affichage */
...
/*
 * Associe un client-data a un window.
*/
...
/*
 * Recupere le client-data associe'e a un window.
*/
...
void ez_main_loop ()
{
    ...
    /* Affiche toutes les fenetres dont l'affichage a ete diffe're' */
 ...
 *
 * Par defaut, ez_error affiche sur stderr. On peut supprimer les messages en 
 * faisant : ez_error_handler = ez_error_ign
 * ou intercepter les messages en changeant ez_error_handler.
*/
...
/*
 * Teste si ez_state est EZ_POST_INIT ou EZ_MAIN_LOOP
 * renvoie 0 succes, -1 erreur. Verbeux
*/
...
/*
 * Destruction d'une fenetre (sans la retirer de la liste des fenetres).
 * Appele'e par ez_window_destroy et ez_win_delete_all.
*/
...
void ez_win_delete (Window win)
{
    ...
    /* Suppression des infos, apres ez_window_dbuf qui les utilise encore */
...
/* Destruction des fenetres restantes. Appele'e a la fin du programme
 * par ez_close_disp si ez_win_delete_final = 1. 
*/
...
/*
 * Appelee par exit (fct memorise'e par atexit dans ez_init)
*/
...
void ez_event_dispatch (Ez_event *ev)
{
    ...
    /* On decode l'evenement. Pour l'ignorer, il suffit de faire return */
...
LRESULT CALLBACK ez_win_proc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        ...
        case WM_CHAR :
        {
            ...
            if (msg == WM_CHAR) {
                if (ez_keychar_convert (wParam, &ksym, &kname, &kstring) < 0) 
                    return 0L;
                /* On memorise la touche pour le prochain WM_KEYUP */
                ezx.key_sym = ksym;
                ezx.key_name = kname;
            ...
            if (ez_is_repetition (lParam)) {
                if (ez_is_modifier (ksym)) {
                    /* On supprime les repetitions des touches Ctrl, ALt et Shift */
                    return 0L;
                } else {
                    /* On simule KeyRelease avant le KeyPress */
        ...
        case WM_SYSKEYUP :
        {
            ...
            if (ez_keydown_convert (wParam, lParam, &ksym, &kname, &kstring) < 0) {
               /* Ce message contient la description du WM_KEYDOWN correspondant, 
                  or c'est la description du precedent WM_CHAR qui est pertinente. */
                ksym = ezx.key_sym;
                kname = ezx.key_name;
...
/*
 * Renvoie 1 si key_sim est une touche Ctrl, Shift ou Alt, 0 sinon.
*/
...
/*
 * Renvoie 1 si la touche est repete'e, 0 sinon.
*/
...
/*
 * Renvoie le point d'insertion dans la liste triee ezx.win_l
*/
...
int ez_win_list_find (Window win)
{
    ...
    while (m2-m1 > 0) {
        ...
        if (win > ezx.win_l[mid])
             m1 = mid+1;   /* recherche dans [mid+1 .. m2] */
        else m2 = mid;     /* recherche dans [m1 .. mid]   */
    }
...
/*
 * Insere le Window dans la liste triee ezx.win_l
 * (win different de None, et win non deja present dans la liste)
...
int ez_win_list_insert (Window win)
{
    ...
    /* Insere en position k */
    if (k < ezx.win_nb)
        memmove ( ezx.win_l+k+1, ezx.win_l+k,
                 (ezx.win_nb-k)*sizeof(Window) );
    ezx.win_nb++;
    ezx.win_l[k] = win;
    return 0;
}
...
/*
 * Supprime le Window de la liste triee ezx.win_l
 * Renvoie 0 succes, -1 erreur.
*/
...
/*
 * Associe une propriete a un window.
 * Renvoie 0 succes, -1 erreur.
*/
...
/*
 * Recupere une propriete associe'e a un window.
 * Renvoie 0 succes, -1 erreur
*/
...
/*
 * Libere la propriete associe'e au window.
 * Renvoie 0 succes, -1 erreur.
*/
...
/*
 * Recupere le Ez_win_info associe' a un window.
 * Renvoie 0 succes, -1 erreur
*/
